...
class AudioPlayerViewModel: UIViewController {
    ...
    var songTrack: Track?
    var delegate: AudioPlayerViewController?
    ...
    func eliminar() -> Void {
        ...
        delegate!.showSimplePopUpAlert("Borrado","Se ha borrado la cancion \(self.songTrack?.title ?? "")")
        print("Delete")
    }
    
    func descargar() -> Void {
        DownloadManager.shared.startDownload(url: URL(string: "https://speed.hetzner.de/100MB.bin")!)
        delegate!.showSimplePopUpAlert("Descarga","Se ha comenzado a descargar \(self.songTrack?.title ?? "")")
    }
    
    func agregarAPlaylist() -> Void {
        playlistSongs.append(songTrack!)
        delegate!.showSimplePopUpAlert("Favoritos","Se ha agregado a favoritos: \(self.songTrack?.title ?? "")")
    }
    ...
    func compartir() -> Void {
        ...
        delegate!.present(shareMenu, animated: true)
        print("Share song")
    }
}
